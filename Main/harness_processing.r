...
#convert integer measure.vars into numeric
  ...
  harness.data[,integers:=lapply(harness.data[,integers,with=FALSE], as.numeric),with=FALSE]
...
#convert integer measure.vars into numeric
  ...
  data[,integers:=lapply(data[,integers,with=FALSE], as.numeric),with=FALSE]
  return(data)
}
